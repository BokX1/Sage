...
const DEFAULT_VARIANTS: VariantSpec[] = [
  ...
  {
    name: 'tool_loop_low_rounds',
    description: 'Reduce tool loop rounds to test latency-first behavior',
    env: {
      AGENTIC_TOOL_MAX_ROUNDS: '1',
    },
  },
  {
    name: 'tool_loop_low_parallel',
    description: 'Reduce read-only tool parallelism for stability checks',
    env: {
...
function computeCompositeScore(summary: SimulationSummary): number {
  ...
  const reliability =
    ...
    judgeCoverageBonus;
  const tooling = summary.toolExecutionRate * 0.15;
