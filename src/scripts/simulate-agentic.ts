...
const ROUTES = ['chat', 'coding', 'search'] as const;
...
async function runSingleSimulation(params: {
  ...
  try {
    ...
    const observedRoute = trace?.routeKind ?? 'single';
    ...
    const judgeVerdict: 'pass' | 'revise' | null = null;
    const judgeIssues: string[] = [];
    const judgeErrorText: string | undefined = undefined;
...
async function main(): Promise<void> {
  ...
  const runIndexes = Array.from({ length: runCount }, (_, index) => index);
  ...
  const rows = await Promise.all(
    runIndexes.map((index) =>
      limiter(async () => {
        const route = ROUTES[Math.floor(rng() * ROUTES.length)] ?? 'chat';
