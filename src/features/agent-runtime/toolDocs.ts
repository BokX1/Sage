...
export const discordMessagesToolDoc = registerRoutedToolDoc({
  ...
  purpose:
    'Read and write Discord message surfaces: message search/history, context windows, final in-channel replies, reactions, polls, and thread creation.',
  useWhen: [
    'The user needs exact message evidence or message-level actions.',
    'You want the final answer delivered directly into Discord.',
    'You need reactions, polls, or threads.',
  ],
  avoidWhen: [
    ...
    'You need admin-only moderation or channel/role/member management; use discord_admin.',
  ],
  routingNotes: [
  ...
  selectionHints: [
    ...
    '  - Polls, threads, and reactions -> create_poll / create_thread / add_reaction / remove_self_reaction.',
...
export const discordAdminToolDoc = registerRoutedToolDoc({
  ...
  routingNotes: [
    ...
    'api is the fallback only when typed admin actions do not cover the job.',
  ],
  selectionHints: [
