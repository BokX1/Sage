...
export type FetchDiscordAttachmentBytesResult =
  ...
  | { kind: 'error'; message: string; mimeType?: string | null; byteLength?: number }
  | { kind: 'ok'; buffer: Buffer; mimeType?: string | null; byteLength?: number };
...
export async function fetchDiscordAttachmentBytes(
  url: string,
  filename: string,
  opts: Partial<FetchDiscordAttachmentBytesOptions> = {},
): Promise<FetchDiscordAttachmentBytesResult> {
  if (!url) {
    return {
      kind: 'skip',
      reason: buildSystemMessage('Attachment URL missing; skipped.'),
  ...
  const maxBytes =
    typeof opts.maxBytes === 'number' && Number.isFinite(opts.maxBytes) ? Math.floor(opts.maxBytes) : 0;
  if (maxBytes <= 0) {
    return {
      kind: 'too_large',
      message: buildSystemMessage(`File '${filename}' omitted due to size limits.`),
    };
  }
  ...
  if (fetched.kind === 'too_large') {
    return { kind: 'too_large', message: fetched.message };
  }
  if (fetched.kind === 'error') {
    return { kind: 'error', message: fetched.message };
  }

  return { kind: 'ok', buffer: fetched.buffer, mimeType: fetched.contentType, byteLength: fetched.byteLength };
}
