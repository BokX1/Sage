...
// Cache parsed channel lists and refresh them when backing config values change.
let cachedBlocklist: Set<string> | null = null;
let cachedAllowlist: Set<string> | null = null;
let cachedBlocklistCsv: string | null = null;
let cachedAllowlistCsv: string | null = null;
...
function getBlocklist(): Set<string> {
  const raw = config.INGESTION_BLOCKLIST_CHANNEL_IDS_CSV;
  if (!cachedBlocklist || cachedBlocklistCsv !== raw) {
    cachedBlocklist = parseChannelList(raw);
    cachedBlocklistCsv = raw;
  }
  return cachedBlocklist;
}

function getAllowlist(): Set<string> {
  const raw = config.INGESTION_ALLOWLIST_CHANNEL_IDS_CSV;
  if (!cachedAllowlist || cachedAllowlistCsv !== raw) {
    cachedAllowlist = parseChannelList(raw);
    cachedAllowlistCsv = raw;
  }
