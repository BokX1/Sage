...
/**
 * Represents the JudgeModelInvoker type.
 */
...
function normalizeRouteKind(routeKind: string): string {
  ...
  if (normalized.startsWith('chat')) return normalized;
  if (normalized.startsWith('coding')) return normalized;
  if (normalized.startsWith('search')) return normalized;
  return 'single';
}
...
async function resolveJudgeModels(input: LlmJudgeInput): Promise<ResolvedJudgeModels> {
  ...
  const defaultModel = (config.CHAT_MODEL || 'kimi').trim();
  const primary = input.primaryModel?.trim() || defaultModel;
  const secondary = input.secondaryModel?.trim() || defaultModel;
  const adjudicator = input.adjudicatorModel?.trim() || defaultModel;
