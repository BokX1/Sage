...
import { filterChannelIdsByMemberAccess, type ChannelPermissionRequirement } from '../utils/discord-channel-access';
import {
...
async function fetchJson(url: string, init: RequestInit, timeoutMs: number): Promise<Record<string, unknown>> {
  ...
  if (!raw.trim()) throw new Error('Response body was empty');
  let parsed: unknown;
  try {
...
export async function lookupGitHubFile(params: {
  ...
  try {
    ...
    throw new Error(
      `github_get_file failed for repo "${params.repo}" path "${params.path}"${refLabel}: ${reason}`,
      { cause: error },
    );
  }
}
...
export async function lookupGitHubCodeSearch(params: {
  ...
  try {
    ...
    if (normalizedRef) {
      try {
        ...
        logger.warn(
          ...
          'github_search_code: unable to derive ref-scoped tree candidates; continuing with search API candidates',
        );
      }
    }
    ...
    return {
      ...
      guidance:
        matches.length > 0
          ? 'Use github_get_file with line ranges around match locations for deeper inspection.'
          : 'No regex matches found in scanned candidates.',
    };
  } catch (error) {
    const reason = error instanceof Error ? error.message : String(error);
    throw new Error(
      `github_search_code failed for repo "${params.repo}" query "${params.query}": ${reason}`,
      { cause: error },
    );
  }
}
