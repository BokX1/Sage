...
- **Active-run steering**: while a task is still `running`, the original requester can reply to Sage's canonical live response message to queue a latest-wins steering interrupt for the next safe reasoning boundary instead of forking a fresh task.
...
| `AGENT_RUN_COMPACTION_TRIGGER_EST_TOKENS` | Estimated prompt tokens that trigger compaction pressure | `64000` |
