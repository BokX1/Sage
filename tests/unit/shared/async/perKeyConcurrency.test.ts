...
import { clearKeyLimit, limitByKey } from '../../../../src/shared/async/perKeyConcurrency';
...
describe('perKeyConcurrency', () => {
  ...
  it('allows reconfiguration after clearing a key', async () => {
    ...
    const results = await Promise.all([limiter2(async () => Promise.resolve('one')), limiter2(async () => Promise.resolve('two'))]);
