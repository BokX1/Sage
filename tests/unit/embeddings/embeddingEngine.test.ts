...
describe('embeddingEngine dimension guard', () => {
  ...
  it('loads when configured dimensions match vector schema', async () => {
    ...
    vi.doMock('../../../src/core/utils/logger', () => ({ logger: mockLogger }));

    const module = await import('../../../src/core/embeddings/embeddingEngine');
  ...
  it('throws if configured dimensions do not match vector(256) schema', async () => {
    ...
    await expect(import('../../../src/core/embeddings/embeddingEngine')).rejects.toThrow(
