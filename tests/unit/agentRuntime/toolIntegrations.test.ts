...
vi.mock('@/core/utils/discord-channel-access', () => ({
...
describe('toolIntegrations', () => {
  ...
  it('decodes GitHub file content', async () => {
    ...
    expect((result as { path: string }).path).toBe('README.md');
    expect(String((result as { content: unknown }).content)).toContain('hello from file');
  });

  it('supports ranged GitHub file lookup with line numbers and per-trace cache reuse', async () => {
    fetchMock.mockResolvedValueOnce({
      ok: true,
      status: 200,
      statusText: 'OK',
      text: async () =>
        JSON.stringify({
          type: 'file',
          encoding: 'base64',
          content: Buffer.from('alpha\nbeta\ngamma\ndelta', 'utf8').toString('base64'),
          html_url: 'https://github.com/acme/repo/blob/main/src/file.ts',
          sha: 'sha-1',
        }),
    } satisfies {
      ok: boolean;
    ...
    const first = await lookupGitHubFile({
      repo: 'acme/repo',
      path: 'src/file.ts',
      startLine: 2,
      endLine: 3,
    ...
    const second = await lookupGitHubFile({
      repo: 'acme/repo',
      path: 'src/file.ts',
      startLine: 4,
      endLine: 4,
    ...
    expect(String((first as { content: unknown }).content)).toContain('2| beta');
    expect(String((first as { content: unknown }).content)).toContain('3| gamma');
    expect((first as { lineStart: number }).lineStart).toBe(2);
    expect((first as { lineEnd: number }).lineEnd).toBe(3);
    expect((first as { hasMoreBefore: boolean }).hasMoreBefore).toBe(true);
    expect((first as { hasMoreAfter: boolean }).hasMoreAfter).toBe(true);
    expect(String((second as { content: unknown }).content)).toContain('delta');
    expect(fetchMock).toHaveBeenCalledTimes(1);
  });

  it('includes repo/path context when GitHub file lookup fails', async () => {
    fetchMock.mockResolvedValueOnce({
      ok: false,
      status: 404,
  ...
  it('searches GitHub code and refines with regex across candidate files', async () => {
    fetchMock
      .mockResolvedValueOnce({
        ok: true,
        status: 200,
            ...
            items: [
              {
                path: 'src/a.ts',
                sha: 'sha-a',
                html_url: 'https://github.com/acme/repo/blob/main/src/a.ts',
                score: 1.2,
              },
              {
                path: 'src/b.ts',
                sha: 'sha-b',
                html_url: 'https://github.com/acme/repo/blob/main/src/b.ts',
                score: 1.1,
              },
            ],
          }),
      ...
      .mockResolvedValueOnce({
        ...
        text: async () =>
          JSON.stringify({
            type: 'file',
            encoding: 'base64',
            content: Buffer.from('const x = 1;\n// marker fix parser', 'utf8').toString('base64'),
            sha: 'sha-a',
            html_url: 'https://github.com/acme/repo/blob/main/src/a.ts',
          }),
      } satisfies {
      ...
      .mockResolvedValueOnce({
        ...
        text: async () =>
          JSON.stringify({
            type: 'file',
            encoding: 'base64',
            content: Buffer.from('const y = 2;\n// nothing here', 'utf8').toString('base64'),
            sha: 'sha-b',
            html_url: 'https://github.com/acme/repo/blob/main/src/b.ts',
          }),
      } satisfies {
    ...
    const result = await lookupGitHubCodeSearch({
      repo: 'acme/repo',
      query: 'marker',
      regex: 'marker\\s+fix',
      traceId: 'trace-2',
    ...
    expect((result as { candidateCount: number }).candidateCount).toBe(2);
    expect((result as { scannedFileCount: number }).scannedFileCount).toBe(2);
  ...
  it('derives ref-scoped code candidates from git tree when search API has no branch matches', async () => {
    fetchMock
      .mockResolvedValueOnce({
        ok: true,
        status: 200,
      ...
      .mockResolvedValueOnce({
        ...
        text: async () =>
          JSON.stringify({
            tree: [
              { path: 'src/feature-only.ts', type: 'blob', sha: 'sha-feature' },
      ...
      .mockResolvedValueOnce({
        ...
        text: async () =>
          JSON.stringify({
            type: 'file',
            encoding: 'base64',
            content: Buffer.from('const branchOnly = true;\n// branch marker', 'utf8').toString(
              'base64',
            ),
            sha: 'sha-feature',
            html_url: 'https://github.com/acme/repo/blob/feature/new-path/src/feature-only.ts',
          }),
      } satisfies {
        ok: boolean;
        status: number;
    ...
    const result = await lookupGitHubCodeSearch({
      repo: 'acme/repo',
      ref: 'feature/new-path',
      query: 'feature-only',
      regex: 'branch\\s+marker',
      maxCandidates: 5,
      maxFilesToScan: 2,
      traceId: 'trace-branch',
    });

    expect((result as { candidateCount: number }).candidateCount).toBe(1);
    expect((result as { scannedFileCount: number }).scannedFileCount).toBe(1);
    expect((result as { matchCount: number }).matchCount).toBe(1);
    expect((result as { candidates: unknown[] }).candidates).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          path: 'src/feature-only.ts',
          source: 'ref_tree',
        }),
      ]),
    );
  ...
  it('rejects invalid regex for GitHub code search', async () => {
    await expect(
      lookupGitHubCodeSearch({
        repo: 'acme/repo',
        query: 'marker',
