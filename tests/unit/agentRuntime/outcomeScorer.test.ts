...
describe('outcomeScorer', () => {
  ...
  it('flags completely empty replies', () => {
    const score = scoreTraceOutcome({
      routeKind: 'chat',
      replyText: '   ', // whitespace only
    });

    expect(score.successLikely).toBe(false);
    expect(score.riskFlags).toContain('empty_reply');
    expect(score.notes).toContain('Reply text is empty.');
  });
  ...
  it('scores healthy responses positively', () => {
    const score = scoreTraceOutcome({
      ...
      qualityJson: { score: 0.9, verdict: 'pass' },
  ...
  it('penalizes tool loop failures', () => {
    const score = scoreTraceOutcome({
      routeKind: 'search',
