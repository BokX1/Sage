...
import {
  clearAllOverrides,
  isLoggingEnabled,
} from '@/core/settings/guildChannelSettings';
...
describe('logging policy', () => {
  ...
  it('refreshes blocklist cache when config changes', () => {
    mockConfig.INGESTION_BLOCKLIST_CHANNEL_IDS_CSV = 'channel-1';
    expect(isLoggingEnabled('guild-1', 'channel-1')).toBe(false);

    mockConfig.INGESTION_BLOCKLIST_CHANNEL_IDS_CSV = 'channel-2';
    expect(isLoggingEnabled('guild-1', 'channel-1')).toBe(true);
    expect(isLoggingEnabled('guild-1', 'channel-2')).toBe(false);
  });
});
