...
} from '@/core/llm/context-budgeter';
...
describe('budgeter', () => {
  ...
  it('applies vision fade to older images', () => {
    ...
    expect(String(result.trimmed[0].content)).toContain('Image omitted from history');
  });
  ...
  it('truncates attachment blocks to the configured max', () => {
    ...
    const messages: LLMChatMessage[] = [{ role: 'user', content }];
