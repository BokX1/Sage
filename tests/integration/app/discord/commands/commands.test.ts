...
vi.mock('discord.js', async () => {
  ...
  return {
    ...
    REST: MockREST,
    Routes: {
      applicationGuildCommands: () => 'guild-route',
      applicationCommands: () => 'global-route',
    },
  };
});
...
describe('Discord command registry', () => {
  ...
  it('does not include removed model commands', async () => {
    const { commandPayloads } = await import('@/app/discord/commands/slash-command-registry');
    const commandNames = commandPayloads.map((command) => command.name).sort();
    ...
    expect(commandNames).toEqual(['join', 'leave', 'ping', 'sage']);
  });

  it('registers commands without network calls', async () => {
    process.env.DEV_GUILD_ID = 'test-guild-id';

    const { registerCommands } = await import('@/app/discord/commands/slash-command-registry');
